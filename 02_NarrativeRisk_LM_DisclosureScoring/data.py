"""Self-contained I/O + on-demand fetchers for the 02_NarrativeRisk_LM_DisclosureScoring quantlet.
...
LICENSE NOTES (for the third-party files this quantlet depends on):
  ...
  - lm_dictionary.csv (Loughran-McDonald Master Dictionary, SRAF):
      Distributed by the Software Repository for Accounting and Finance
      (SRAF) at the University of Notre Dame, free for academic research
      use. Cite Loughran, T., & McDonald, B. (2011). "When is a Liability
...
LM_DICT_PATH = HERE / "lm_dictionary.csv"
...
# ---------------------------------------------------------------------------
# Loughran-McDonald master dictionary (SRAF)
# ---------------------------------------------------------------------------

_LM_CANDIDATE_URLS = [
    "https://sraf.nd.edu/wp-content/uploads/2024/02/Loughran-McDonald_MasterDictionary_1993-2023.csv",
    "https://sraf.nd.edu/wp-content/uploads/2023/09/Loughran-McDonald_MasterDictionary_1993-2022.csv",
]

_LM_MIN_BYTES = 10_000  # anything smaller is almost certainly an error page
_LM_USER_AGENT = "AMLEF-Narratives-Quantlet/1.0 (academic research)"


def _looks_like_html(payload: bytes) -> bool:
    """Cheap heuristic: redirected landing pages start with <!DOCTYPE / <html."""
    head = payload[:512].lstrip().lower()
    return head.startswith(b"<!doctype") or head.startswith(b"<html")


def _write_lm_dictionary_csv(target: Path) -> None:
    """Download the LM master dictionary, walking the candidate URL list."""
    last_error = None
    for url in _LM_CANDIDATE_URLS:
        try:
            req = urllib.request.Request(url, headers={"User-Agent": _LM_USER_AGENT})
            with urllib.request.urlopen(req, timeout=60) as response:
                payload = response.read()
        except Exception as exc:  # noqa: BLE001  -- we want to fall through to next URL
            last_error = f"{type(exc).__name__}: {exc}"
            print(f"  candidate {url} failed: {last_error}")
            continue
        if len(payload) < _LM_MIN_BYTES:
            last_error = f"payload too small ({len(payload)} bytes < {_LM_MIN_BYTES})"
            print(f"  candidate {url} failed: {last_error}")
            continue
        if _looks_like_html(payload):
            last_error = "response looks like an HTML landing page (not the CSV)"
            print(f"  candidate {url} failed: {last_error}")
            continue
        target.write_bytes(payload)
        print(f"  wrote {len(payload):,} bytes from {url} -> {target.name}")
        return

    raise SystemExit(
        "ERROR: could not download Loughran-McDonald master dictionary from any "
        "candidate URL.\n"
        f"Last error: {last_error}\n"
        "Manual download from "
        "https://sraf.nd.edu/loughranmcdonald-master-dictionary/ and place the "
        f"CSV at {target}."
    )
...
def _ensure_lm_dictionary(path: Path = LM_DICT_PATH) -> None:
    if not path.exists():
        print(f"[lm_dictionary] {path.name} missing; fetching from SRAF ...")
        _write_lm_dictionary_csv(path)
...
def load_lm_dictionary(path: Path = LM_DICT_PATH) -> pd.DataFrame:
    """Load slim Loughran-McDonald dictionary.

    Skips comment lines beginning with '#'. Returns DataFrame with columns
    ['word', 'category'] where category in
    {negative, positive, uncertainty, litigious, strong_modal, weak_modal, constraining}.
    Words are lowercase ASCII.

    Auto-fetches from SRAF (Notre Dame) on first call if the CSV is missing.
    """
    _ensure_lm_dictionary(path)
    rows = []
    with open(path, "r", encoding="utf-8") as f:
        reader = csv.DictReader(row for row in f if not row.startswith("#"))
        for r in reader:
            rows.append({"word": r["word"].strip().lower(), "category": r["category"].strip()})
    return pd.DataFrame(rows)
...
def main() -> None:
    ...
    print(f"data.py for 02_NarrativeRisk_LM_DisclosureScoring (writing to {HERE})")
    ...
    lm_target = HERE / "lm_dictionary.csv"
    if args.force or not lm_target.exists():
        print("[lm_dictionary]")
        _write_lm_dictionary_csv(lm_target)
    else:
        print(f"[lm_dictionary] {lm_target.name} present; skipping (use --force to re-fetch)")
