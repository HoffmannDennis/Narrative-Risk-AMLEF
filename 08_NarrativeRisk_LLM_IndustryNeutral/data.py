"""Self-contained I/O + on-demand fetchers for the 08_NarrativeRisk_LLM_IndustryNeutral quantlet.
...
FIRM_LIST_PATH = HERE / "firm_list.csv"
...
def load_firm_list(path: Path = FIRM_LIST_PATH) -> pd.DataFrame:
    """Load firm_list.csv and return a DataFrame with columns [ticker, sector].

    The CSV ships with the quantlet (sector mapping for the top-30 universe);
    this loader does no auto-fetch — if the file is missing, that's an error.
    """
    df = pd.read_csv(path)
    df.columns = [c.lower() for c in df.columns]
    return df[["ticker", "sector"]].copy()
...
def main() -> None:
    ...
    print(f"data.py for 08_NarrativeRisk_LLM_IndustryNeutral (writing to {HERE})")
